...
EXTRACT_QUEUE = os.environ['QUEUE']
DDB = os.environ['DDB']
...
def countFastaHeaders(filePath):
    count = 0
...
def lambda_handler(event, context):
    ...
    ddb_client.put_item(
        ...
        Item={
            "JobID": {"S": jobid},
            "TotalInvocations": {"N": str(totalInvocations)},
            "CompletedCount": {"N": "0"},
        }
    )

    for i in range(1, totalInvocations + 1):
        body ={ 
            "Genome": accession, 
            "Sequence": sequence, 
            "JobID": jobid,
        ...
        sqs_send_message(EXTRACT_QUEUE, json_object) 
