...
# Extract Off-targets
def performExtraction(accession, tmp_fasta_dir, context, Invocation, totalInvocations):
    
    print("\nExtracting Offtargets...")

    tmp_dir = get_tmp_dir()
    offtargetfn = os.path.join(tmp_dir,f"{accession}({Invocation}).offtargets")
    print(f"Creating: {offtargetfn}")

    # Lambda code
    extractOfftargets.startSequentalprocessing([tmp_fasta_dir], offtargetfn, 1, 100, context, Invocation, totalInvocations)
...
def lambda_handler(event, context):
    ...
    totalInvocations = args['TotalInvocations']
    ...
    # Create offtarget files
    performExtraction(accession, tmp_dir, context, Invocation, totalInvocations)
    ...
    completed = int(resp["Attributes"]["CompletedCount"]["N"])
    total = int(resp["Attributes"]["TotalInvocations"]["N"])

    if completed == total:
        sqs_send_message(MERGER_QUEUE, json_object)
